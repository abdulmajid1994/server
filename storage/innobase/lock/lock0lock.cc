...
static void
lock_rec_free_all_from_discard_page(const page_id_t id, lock_t *lock,
						...
						is the first record moved */
{
  ...
  const page_id_t new_id{new_block->page.id()};
  {
    ...
    for (lock_t *lock= g.lock1(); lock; lock= lock_rec_get_next_on_page(lock))
    {
						...
						discarded */
{
  ...
  lock_rec_free_all_from_discard_page(l, g.lock1(), lock_sys.rec_hash);
...
void lock_update_copy_and_discard(const buf_block_t &new_block, page_id_t old)
{
  ...
  lock_rec_move(g.cell1(), new_block, id, g.cell2(), old,
                PAGE_HEAP_NO_SUPREMUM, PAGE_HEAP_NO_SUPREMUM);
  lock_rec_free_all_from_discard_page(old, g.lock2(), lock_sys.rec_hash);
}
                            ...
                            const page_id_t right)
{
  ...
  lock_rec_move(g.cell1(), left, l, g.cell2(), right,
                PAGE_HEAP_NO_SUPREMUM, PAGE_HEAP_NO_SUPREMUM);
  lock_rec_free_all_from_discard_page(right, g.lock2(), lock_sys.rec_hash);
						...
						which will be discarded */
{
	...
	if (lock_sys_t::get_first(g.cell2(), page_id)) {
		...
		lock_rec_free_all_from_discard_page(page_id, g.lock2(),
						    lock_sys.rec_hash);
	} else {
